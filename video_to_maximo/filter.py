...
class OneEuroFilter:
    ...
    def filter(self, x: float, t: float) -> float:
        ...
        dx_filtered = (
            alpha_dx * dx + (1 - alpha_dx) * self._dx_prev
            if self._dx_prev is not None
            else dx
        )
...
class Smoother:
    ...
    def __init__(self, config: FilterConfig = None, z_config: FilterConfig = None):
        """
        Initialize smoother.
    ...
    def filter_with_missing(
        self, landmarks: List[List[float]], timestamps_ms: List[float]
        ...
        for i, (landmarks_frame, timestamp_ms) in enumerate(
            zip(landmarks, timestamps_ms)
        ):
    ...
    def smooth(self, data: np.ndarray) -> np.ndarray:
        ...
        for axis in range(3):
            smoothed[:, axis] = savgol_filter(
                data[:, axis], self.window_size, self.poly_order
            )

        return smoothed

    def smooth_sequence(
        self, landmark_sequence: List[List[List[float]]]
    ) -> List[List[List[float]]]:
        """
        ...
        for lm_idx in range(33):
            for axis in range(3):
                smoothed[:, lm_idx, axis] = self.smooth(data[:, lm_idx, axis])
