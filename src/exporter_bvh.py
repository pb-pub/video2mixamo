...
from skeleton import Skeleton, SKELETON_BONES, get_landmark_indices
...
class BVHExporter:
    ...
    def export(
        self,
        output_path: str,
        frame_rotations: List[RotationResult],
        fps: float = 30.0
    ) -> None:
        """
    ...
    def _write_bone_recursive(self, f, bone_name: str, depth: int) -> int:
        ...
        f.write(f"Zrotation Xrotation Yrotation\n")
    ...
    def _write_motion(self, f, frame_rotations: List[RotationResult], fps: float) -> None:
    ...
    def _get_empty_frame_values(self, bone_names: List[str]) -> List[float]:
        ...
        for bone_name in bone_names:
            ...
            if bone_name == "Hips" or self.skeleton.get_bone(bone_name).parent_name is None:
...
def test_exporter():
    ...
    for frame_idx in range(3):
        ...
        result = computer.compute_rotations(test_landmarks, timestamp_ms=frame_idx * 1000 // 30)
