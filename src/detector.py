...
class PoseLandmarker:
    ...
    def _parse_result(
        self,
        mp_result: vision.PoseLandmarkerResult,
        timestamp_ms: float
    ) -> PoseResult:
        ...
        if mp_result.segmentation_masks and output_segmentation_masks:
    ...
    with PoseLandmarker(model_path=model_path) as detector:
        ...
        try:
            while True:
                ...
                if result.success:
                    print(f"Frame {frame_count}: Detected {len(result.pose_landmarks)} landmarks")
